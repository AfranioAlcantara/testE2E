import cadastro from '../../fixtures/cadastroSwagger.json'
...
it('valida a mudança do cadastro', () => {
    cy.request({
        method: 'GET',
            url:'https://petstore.swagger.io/v2/user/nil'
    }).then((response)=>{
        expect(response.status).to.eq(200)
        expect(JSON.stringify(response.body)).to
        .equal(JSON.stringify(cadastro))
    })
});
